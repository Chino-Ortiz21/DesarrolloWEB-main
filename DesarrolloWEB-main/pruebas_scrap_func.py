...
def parse_offers(url, categoria_scrapeada):
    try:
    ...
    for a in links:
        ...
        # Limpiar texto para extraer nombre y descripción
        texto_limpio = re.sub(r"\$[\d\.]+", "", texto)
        texto_limpio = texto_limpio.replace("Agregar", "").replace("Despacho", "").strip()

        palabras = texto_limpio.split()
        nombre = " ".join(palabras[:8])  # Primeras palabras como nombre (ajustable)
        descripcion = " ".join(palabras[8:]) if len(palabras) > 8 else ""

        productos.append({
            "nombre": nombre,
            "descripcion": descripcion if descripcion else nombre,
            "precio_actual": precio_actual,
            "precio_anterior": precio_anterior,
            "descuento": f"{descuento}%" if descuento else "N/D",
            "link": url_producto,
            "categoria": CATEGORY_MAP.get(categoria_scrapeada, "otros")
        })
...
for categoria, url in offer_pages.items():
    ...
    datos = parse_offers(url, categoria)
    if not datos:
